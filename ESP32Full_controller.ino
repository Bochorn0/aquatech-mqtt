...
const int FLOW_PIN_PROD = 18;  // ⚠️ Cambiado de 15 a 18 (pin 15 ahora es RELAY_FLUSH_PIN)
const int FLOW_PIN_RECH = 4;
const int RELAY_PIN = 2;  // ✅ Cambiado de 16 a 2 - Relay bomba
const int RELAY_FLUSH_PIN = 15;  // ✅ Cambiado de 17 a 15 - Relay para flush programado
...
void updateRelay() {
  ...
  if (relayState != previousRelayState) {
    logWithTimestamp("RELAY", "Estado final: %s (Pin %d: %s)", 
           relayState ? "ON" : "OFF",
           RELAY_PIN,
...
void executeFlush() {
  ...
  delay(50);
  int pinFlushCheck = digitalRead(RELAY_FLUSH_PIN);
  logger("FLUSH", "✅ Relay FLUSH (Pin %d): %s → %s",
         RELAY_FLUSH_PIN,
         pinFlushCheck == HIGH ? "HIGH" : "LOW",
         pinFlushCheck == HIGH ? "ON ✓" : "OFF ❌");
  ...
  updateRelay();
  delay(50);
  int pinBombaCheck = digitalRead(RELAY_PIN);
  logger("FLUSH", "✅ Relay BOMBA (Pin %d): %s → %s",
         RELAY_PIN,
         pinBombaCheck == HIGH ? "HIGH" : "LOW",
         pinBombaCheck == HIGH ? "ON ✓" : "OFF ❌");
  
  // Verificar que ambos están encendidos
  if (pinBombaCheck == HIGH && pinFlushCheck == HIGH) {
  ...
  while (millis() - startFlush < FLUSH_DURATION) {
    ...
    if (now - lastSecondLog >= 1000) {
      if (elapsedSeconds < 5 || elapsedSeconds >= 25) {
        int pBomba = digitalRead(RELAY_PIN);
        int pFlush = digitalRead(RELAY_FLUSH_PIN);
        logger("FLUSH", "⏱️ Seg %lu | Pin%d (Bomba): %s | Pin%d (Flush): %s | Estado: %s", 
               elapsedSeconds,
               RELAY_PIN,
               pBomba == HIGH ? "ON ✓" : "OFF ❌",
               RELAY_FLUSH_PIN,
               pFlush == HIGH ? "ON ✓" : "OFF ❌",
               (pBomba == HIGH && pFlush == HIGH) ? "✓ AMBOS ON" : "❌ ERROR");
      }
      lastSecondLog = now;
    }
  ...
  delay(50);
  int pinFlushFinal = digitalRead(RELAY_FLUSH_PIN);
  logger("FLUSH", "✅ Relay FLUSH (Pin %d) desactivado: %s → %s",
         RELAY_FLUSH_PIN,
         pinFlushFinal == LOW ? "LOW" : "HIGH",
         pinFlushFinal == LOW ? "OFF ✓" : "ON ❌");
  ...
  updateRelay();
  delay(50);
  int pinBombaFinal = digitalRead(RELAY_PIN);
  logger("FLUSH", "📊 Relay BOMBA (Pin %d) final: %s → %s (según válvulas)",
         RELAY_PIN,
         pinBombaFinal == HIGH ? "HIGH" : "LOW",
         pinBombaFinal == HIGH ? "ON" : "OFF");
...
void setup() {
  ...
  RELAY_STATE = relayState;
  logger("RELAY", "Estado inicial Relay Principal (Pin %d): OFF (LOW)", RELAY_PIN);
  
  // Relay flush OFF inicialmente
  digitalWrite(RELAY_FLUSH_PIN, LOW);  // ✅ Relay 2 OFF (lógica normal: LOW=OFF)
  RELAY_FLUSH_STATE = false;
  isFlushActive = false;
  logger("FLUSH", "Estado inicial Relay Flush (Pin %d): OFF (LOW)", RELAY_FLUSH_PIN);
  
  // Verificar estado físico de los pines
  delay(100);
  int pinBombaState = digitalRead(RELAY_PIN);
  int pinFlushState = digitalRead(RELAY_FLUSH_PIN);
  logger("RELAY", "✓ Verificación Pin %d (Bomba): %s (esperado: LOW=OFF)", RELAY_PIN, pinBombaState == LOW ? "LOW" : "HIGH");
  logger("FLUSH", "✓ Verificación Pin %d (Flush): %s (esperado: LOW=OFF)", RELAY_FLUSH_PIN, pinFlushState == LOW ? "LOW" : "HIGH");
