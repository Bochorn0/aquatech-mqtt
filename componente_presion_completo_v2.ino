...
const int N_SAMPLES = 20;                      // Aumentado para mejor filtrado
const int MEDIAN_SAMPLES = 15;                 // Muestras para mediana (elimina outliers)
...
const float FILTER_ALPHA = 0.3;                // Factor de suavizado (0.0-1.0, menor = más suave)
const float MAX_PRESSURE_CHANGE = 5.0;         // Cambio máximo permitido entre lecturas (psi) - detecta ruido
const int STABILITY_CHECKS = 3;                // Número de lecturas consistentes requeridas
...
float pressure1_filtered = 0.0f;  // Valor filtrado
float voltage1 = 0.0f;
int raw1 = 0;
bool valid1 = false;
bool connected1 = false;
float lastStablePressure1 = 0.0f;  // Última presión estable

// Salida (Pin 33)
float pressure2_psi = 0.0f;
float pressure2_filtered = 0.0f;  // Valor filtrado
float voltage2 = 0.0f;
int raw2 = 0;
bool valid2 = false;
bool connected2 = false;
float lastStablePressure2 = 0.0f;  // Última presión estable
...
// ================== SENSOR FUNCTIONS ==================
// Función auxiliar para ordenar array (para mediana)
...
// Leer con mediana para eliminar outliers
float readMedianRaw(int pin, int samples) {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    delay(3);  // Aumentado para mejor estabilización
  }
  
  // Ordenar y retornar mediana (elimina outliers)
  sortArray(readings, samples);
  int medianIndex = samples / 2;
  return (float)readings[medianIndex];
}

// Leer promedio con mejor estabilización
float readAvgRaw(int pin, int samples) {
  ...
  for (int i = 0; i < samples; i++) {
    sum += analogRead(pin);
    delay(3);  // Aumentado de 2ms a 3ms para mejor estabilización
  }
  return (float)sum / samples;
}

// Leer con mediana + promedio para máximo filtrado
float readFilteredRaw(int pin) {
  // Primero usar mediana para eliminar outliers
  float median = readMedianRaw(pin, MEDIAN_SAMPLES);
  
  // Luego promediar alrededor de la mediana
  float sum = 0;
  int count = 0;
  float tolerance = 50.0;  // Tolerancia en valores raw (aprox 0.04V)
  ...
  for (int i = 0; i < N_SAMPLES; i++) {
    ...
    // Solo incluir lecturas cercanas a la mediana
    if (abs(reading - median) <= tolerance) {
      sum += reading;
      count++;
    }
    delay(3);
  ...
  // Si no hay suficientes lecturas válidas, usar mediana
...
void readPressureSensors() {
    ...
    connected2 = true;
    valid2 = true;
    pressure1_filtered = pressure1_psi;
    pressure2_filtered = pressure2_psi;
    
    // Calcular diferencia
    PRESSURE_DIFFERENCE = abs(pressure1_psi - pressure2_psi);
    
  #else
    // ===== LECTURA REAL DE SENSORES CON FILTRADO MEJORADO =====
    
    // Leer Pin 35 (Entrada) con filtrado robusto
    raw1 = readFilteredRaw(PRESSURE_PIN_1);
    voltage1 = raw1 * (3.3 / 4095.0);
    
    // Validar voltaje primero
    connected1 = (voltage1 >= MIN_VOLTAGE && voltage1 <= MAX_VOLTAGE);
    
    if (connected1) {
      // Calcular presión
      float newPressure1 = raw1 * RAW_TO_PSI;
      
      // Validar rango de presión
      if (newPressure1 >= MIN_PSI && newPressure1 <= MAX_PSI) {
        // Validar estabilidad: detectar cambios bruscos (ruido)
        if (lastStablePressure1 > 0.0) {
          float pressureChange = abs(newPressure1 - lastStablePressure1);
          
          if (pressureChange <= MAX_PRESSURE_CHANGE) {
            // Cambio razonable - aplicar filtro exponencial
            if (pressure1_filtered == 0.0) {
              pressure1_filtered = newPressure1;
            } else {
              // Filtro exponencial móvil (suavizado)
              pressure1_filtered = pressure1_filtered * (1.0 - FILTER_ALPHA) + newPressure1 * FILTER_ALPHA;
            }
            pressure1_psi = pressure1_filtered;
            lastStablePressure1 = pressure1_psi;
            valid1 = true;
          } else {
            // Cambio muy grande - probablemente ruido, mantener valor anterior
            #if ENABLE_SERIAL_LOGS
            Serial.printf("[SENSOR1] Cambio brusco detectado: %.2f -> %.2f psi (ignorado)\n", 
                         lastStablePressure1, newPressure1);
            #endif
            pressure1_psi = lastStablePressure1;  // Mantener último valor estable
            valid1 = true;
          }
        } else {
          // Primera lectura válida
          pressure1_filtered = newPressure1;
          pressure1_psi = newPressure1;
          lastStablePressure1 = newPressure1;
          valid1 = true;
        }
      } else {
      ...
      valid1 = false;
      pressure1_psi = 0.0;
      pressure1_filtered = 0.0;
    }
    
    // Leer Pin 33 (Salida) con filtrado robusto
    raw2 = readFilteredRaw(PRESSURE_PIN_2);
    voltage2 = raw2 * (3.3 / 4095.0);
    
    // Validar voltaje primero
    connected2 = (voltage2 >= MIN_VOLTAGE && voltage2 <= MAX_VOLTAGE);
    
    if (connected2) {
      // Calcular presión
      float newPressure2 = raw2 * RAW_TO_PSI;
      
      // Validar rango de presión
      if (newPressure2 >= MIN_PSI && newPressure2 <= MAX_PSI) {
        // Validar estabilidad: detectar cambios bruscos (ruido)
        if (lastStablePressure2 > 0.0) {
          float pressureChange = abs(newPressure2 - lastStablePressure2);
          
          if (pressureChange <= MAX_PRESSURE_CHANGE) {
            // Cambio razonable - aplicar filtro exponencial
            if (pressure2_filtered == 0.0) {
              pressure2_filtered = newPressure2;
            } else {
              // Filtro exponencial móvil (suavizado)
              pressure2_filtered = pressure2_filtered * (1.0 - FILTER_ALPHA) + newPressure2 * FILTER_ALPHA;
            }
            pressure2_psi = pressure2_filtered;
            lastStablePressure2 = pressure2_psi;
            valid2 = true;
          } else {
            // Cambio muy grande - probablemente ruido, mantener valor anterior
            #if ENABLE_SERIAL_LOGS
            Serial.printf("[SENSOR2] Cambio brusco detectado: %.2f -> %.2f psi (ignorado)\n", 
                         lastStablePressure2, newPressure2);
            #endif
            pressure2_psi = lastStablePressure2;  // Mantener último valor estable
            valid2 = true;
          }
        } else {
          // Primera lectura válida
          pressure2_filtered = newPressure2;
          pressure2_psi = newPressure2;
          lastStablePressure2 = newPressure2;
          valid2 = true;
        }
      } else {
...
void enviarDatos(const char* productIdParam, float pressure1PSI, float pressure2PSI, float pressureDiff, float temperature) {
  ...
  jsonPayload += "\"voltage_in\":" + String(voltage1, 3) + ",";  // Voltaje sensor entrada
  jsonPayload += "\"voltage_out\":" + String(voltage2, 3) + ",";  // Voltaje sensor salida
...
void setup() {
  ...
  // Configurar ADC con mejor estabilidad
  analogReadResolution(12);
  analogSetAttenuation(ADC_11db);
  analogSetPinAttenuation(PRESSURE_PIN_1, ADC_11db);
  analogSetPinAttenuation(PRESSURE_PIN_2, ADC_11db);
  
  // Estabilizar ADC con lecturas iniciales (descarta primeras lecturas que suelen ser inestables)
  for (int i = 0; i < 5; i++) {
  ...
  pressure1_filtered = 0.0;
  pressure2_filtered = 0.0;
  lastStablePressure1 = 0.0;
  lastStablePressure2 = 0.0;
